    ...
    while not (service.stop):
        ...
        if hourglass_state == -1:
            ...
            if ir.ir[1] > distance_to_regbot + 0.1: # Add a little, in case of noise
                service.send(service.topicCmd + "ti/rc","0.35 0.0") # (forward m/s, turn-rate rad/sec)
                hourglass_state = 1
                pose.tripBreset()
            ...
            if edge.lineValidCnt > 0 or pose.tripBtimePassed() > 7:
                service.send(service.topicCmd + "ti/rc","0.0 0.7") # (forward m/s, turn-rate rad/sec)
                hourglass_state = 2
                pose.tripBreset()
            ...
            print(heading)
            if pose.tripBh > np.pi*0.2:
                service.send(service.topicCmd + "ti/rc","0.0 0.0") # (forward m/s, turn-rate rad/sec)
                hourglass_state = 3
                edge.lineControl(0.13, 1.5) # m/s and position on line -2.0..2.0
        elif hourglass_state == 3:
            print(heading)
            if heading <= desired_stop_heading:# or ir.ir[0] < distance_to_gate:
                hourglass_state=4
